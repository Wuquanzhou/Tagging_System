...
<!--                             </form> -->
                            ...
                            <?php
                                ...
                                //$con=mysql_connect("219.245.4.239","root","12345");
                                if (!$con)
                                  {
                                  die('Could not connect: ' . mysql_error());
                                  }

                                mysql_select_db("lables", $con);    //选取数据库
                                mysql_query("set names utf8");      //设定字符集

                                $sql1 = "select * from register_post where register_post.post_id = (select post_id from register where register_name='".$login['username']."')";
                           ...
                           ?>
                                <li class="active">
                                    <a href="">
                                        <i class="fa fa-dashboard"></i> <span><?php echo $row1['rule_name']?></span>
                                    </a>
                                </li>
                            <?php
                                    }
                            ...
                            ?>
                                <li>
                                    <a href="../patient/up_picv.php">
                                        <i class="fa fa-gavel"></i> <span><?php echo $row1['rule_name']?></span>
                                    </a>
                                </li>
                            <?php
                                    }
...
<!--                                         <button class="btn btn-sm btn-default"><i class="fa fa-search"></i></button> -->
                            ...
                            <form action="../lable/lable.php" enctype="multipart/form-data" method="post">
                              ...
                              <tbody>
                    <?php
                    $sql2 = "select post_id from register where register_name='".$login['username']."'";
                    $result2 = mysql_query($sql2);
                    $row2 = mysql_fetch_array($result2);

                    include_once("../page/Page.php");   //引入类

                    ///////////////////////////////////////////////////////////////////////
                    $PAGE_SIZE=8;            //设置每页显示的数目
                    ...
                    $pageSupport->__set("sql","select * from upload_labler ");
                    ...
                    if ($pageSupport->current_records > 0) //如果数据不为空，则组装数据
                    {
                        for ($i=0; $i<$pageSupport->current_records; $i++)
                        {
                            ...
                            $labler = $pageSupport->result[$i]["labler"];
                            $labler_count = $pageSupport->result[$i]["labler_count"];

                            //$part=substr($content,0,400);
                            //循环输出每条数据
                            if($row2['post_id'] == 'p001'){
                                echo '<tr>
                                  <td>'.$id.'</td>
                                  <td>'.$name.'<button name="mod_name" value = "'.$id.'" style="padding: 1px 5px;" class="btn btn-default btn-xs"><i class="fa fa-pencil"></i></button></td>
                                  <td>'.$labler.'</td>';
                            }else{
                                echo '<tr>
                                  <td>'.$id.'</td>
                            ...
                            ?>
                                  <?php
                                     if($labler_count < 3){
                                       echo '<td><span class="label label-danger">未完成</span></td>
                                             <td><button name="lable" value = "'.$id.'" class="btn btn-default btn-xs"><i class="fa fa-pencil"></i></button></td>
                                        </tr>
                                       ';
                                     }else {
                                         $sql = "select * from uploads where userid='".$id."' and labler='".$login['username']."'";
                                         $result = mysql_query($sql);
                                         $num = mysql_num_rows($result);

                                         $sql1 = "select * from uploads where userid='".$id."'";
                                         $result1 = mysql_query($sql1);
                                         $num1 = mysql_num_rows($result1);
                                         if($num1 >= 4 && $num <= 1){
                                             echo '<td><span class="label label-success">完成</span></td>
                                               <td><button name="check_lable" value = "'.$id.'" style="padding: 1px 5px;" class="btn btn-default btn-xs"><i class="fa fa-check"></i></button>
                                             ...
                                             echo '<td><span class="label label-success">完成</span></td>
                                               <td><button name="check_lable" value = "'.$id.'" style="padding: 1px 5px;" class="btn btn-sm btn-default"><i class="fa fa-search"></i></button>
                                               &nbsp&nbsp&nbsp<button name="lable" value = "'.$id.'" class="btn btn-default btn-xs"><i class="fa fa-pencil"></i></button>
                                               </td>
                                        </tr>
                                       ';
                                         }
